...
!----------------------------------------------------------------------------------------------- 
  ...
  DO m=-mpolb,mpolb
     DO n=-ntorb,ntorb
       ...
       vdmn(n,m)=bmn_e(n,m)*(m*Bzeta-n*nzperiod*Btheta)/aiBtpBz
     END DO
  END DO
  ...
  DO m=-mpolb,mpolb 
     ...
     END DO
  END DO
  fact=vdconst(jv)/(2.*borbic(0,0))
  D11=(16./3.)*D11*(nu(jv)/2.)*fact*fact
  ...
  IF(DEBUG) THEN
     IF(cmul_PS.GT.0) THEN
        WRITE(10000+myrank,'("1 ",6(1pe13.5))') nu(jv)/v(jv)/2,Epsi/v(jv)*psip,G11dkes,&
             & weight(jv)/fdkes(jv),weight(jv)/fdkes(jv)*v(jv)*v(jv)
     ELSE
        WRITE(10000+myrank,'("0 ",6(1pe13.5))') nu(jv)/v(jv)/2,Epsi/v(jv)*psip,G11dkes,&
             & weight(jv)/fdkes(jv),weight(jv)/fdkes(jv)*v(jv)*v(jv)
     END IF
  END IF
